...
export 'package:zego_beauty_effects/define.dart';
...
class ExpressService {
  ...
  Future<void> startPlayingStream(String streamID) async {
    ...
    final userInfo = getUserInfo(userID ?? '');
    if (userInfo != null) {
  ...
  Future<void> onRoomStreamUpdate(String roomID, ZegoUpdateType updateType,
    ...
    for (final stream in streamList) {
      if (updateType == ZegoUpdateType.Add) {
        ...
        var userInfo = getUserInfo(stream.user.userID);
        if (userInfo == null) {
        ...
        try {
          ...
          final isMicOn = (extraInfoMap['mic'] == 'on') ? true : false;
          final isCameraOn = (extraInfoMap['cam'] == 'on') ? true : false;
        ...
        final userInfo = getUserInfo(stream.user.userID);
        userInfo?.streamID = '';
  ...
  void onRoomUserUpdate(
    ...
    if (updateType == ZegoUpdateType.Add) {
      for (final user in userList) {
        var userInfo = getUserInfo(user.userID);
        if (userInfo == null) {
